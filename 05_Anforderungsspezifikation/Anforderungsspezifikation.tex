...
Jeder Bomberman hat einen eindeutigen Namen (name) und eine Anzahl gewonnen Runden (score).
\end{tabularx}
...
& Durch dieses Powerup ist es dem Bomberman möglich mehr als eine Bombe zu platzieren, dabei gibt es ein Maxiumum von 8 Bomben (maxBombs) welche zur gleichen Zeit gelegt werden können.
\end{tabularx}
\end{table}
...
& Durch dieses Powerup beschleunigt sich die Fortbewegungsgeschwindigkeit (movementSpeed) eines Bomberman.
\end{tabularx}
...
& Dieses Powerup gibt der Bombe einen grösseren Sprengradius (bombPower) -> (in alle 4 Himmelsrichtungen) -> (Explosion).
